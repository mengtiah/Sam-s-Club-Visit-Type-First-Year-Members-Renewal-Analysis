...
write.csv(all, file = 'all_origin.csv')  ###save the file that include initial discreption

all_sel= all%>%
  select( -COHORT_MONTH, 
         -MEMBERSHIP_TYPE_DESC, -TENURE_GRP, -LAST_RENEW_DATE,
         -RENEW_DATE, -NEXT_RENEW_DATE,-PLUS_MEMBERSHIP_IND,
...
all_sel$PLUS_STATUS_BEFORE_REN = ifelse(all_sel$PLUS_STATUS_BEFORE_REN == 'BASE',0,1)
