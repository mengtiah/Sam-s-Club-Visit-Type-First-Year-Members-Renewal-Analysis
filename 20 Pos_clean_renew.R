...
r_join_group = r_join %>%
  ...
  summarise(sum_sales = sum(TENDER_AMT),
            count_pay = n())

x = r_join_group 
y = x %>%
  select(MEMBERSHIP_ID, TENDER_TYPE_DESC,sum_sales, count_pay)

write.csv(y, file = 'stack_tender.csv')
